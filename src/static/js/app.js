document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateControls() {
    ...
    solveTspBtn.disabled =
      !algorithmSelect.value || !pointsFileInput.files.length;
  }
  ...
  loadNetworkBtn.addEventListener("click", async function () {
    ...
    try {
      ...
      showStatus(
        `✅ Red cargada: ${data.stats.nodes} nodos, ${data.stats.edges} conexiones`
      );
      pointsFileInput.disabled = false;
      updateControls();
      initMap();

      if (data.visualization_data?.edges) {
        roadNetworkLayer = L.layerGroup();
        data.visualization_data.edges.forEach((edge) => {
  ...
  loadPointsBtn.addEventListener("click", async function () {
    ...
    try {
      ...
      if (data.visualization_data) {
        ...
        data.visualization_data.forEach((p) => {
          const marker = L.circleMarker(
            [p.geometry.coordinates[1], p.geometry.coordinates[0]],
            {
              radius: p.properties.radius || 6,
              fillColor: p.properties.color || "#e74c3c",
              color: "#fff",
              weight: 1,
              fillOpacity: 1,
            }
          ).bindPopup(p.properties.popup);
  ...
  solveTspBtn.addEventListener("click", async function () {
    ...
    if (algorithm === "genetic") {
      params.population_size = parseInt(
        document.getElementById("population-size").value
      );
      params.generations = parseInt(
        document.getElementById("generations").value
      );
      params.mutation_rate =
        parseInt(document.getElementById("mutation-rate").value) / 100;
    }
    ...
    resultsContainer.innerHTML = `
            <div class="result-card">
                ...
                <div class="result-grid">
                    ...
                    <div>${
                      data.execution_time
                        ? data.execution_time.toFixed(2) + " segundos"
                        : "N/A"
                    }</div>
                </div>
            </div>
  ...
  function renderMap(data) {
    ...
    const center = data.center || [
      data.path_coordinates[0][0],
      data.path_coordinates[0][1],
    ];
      ...
      data.visualization_data.points.forEach((point, index) => {
        const marker = L.circleMarker(
          [point.geometry.coordinates[1], point.geometry.coordinates[0]],
          {
            radius: 6,
            fillColor:
              point.properties.color || (index === 0 ? "#2ecc71" : "#e74c3c"),
            color: "#fff",
            weight: 1,
            fillOpacity: 1,
          }
    ...
    if (data.path_coordinates) {
      const routeCoords = data.path_coordinates.map((coord) => [
        coord[0],
        coord[1],
      ]);
