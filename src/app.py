    ...
    try:
        for event, elem in ET.iterparse(filepath, events=('start', 'end')):
            if event == 'start' and elem.tag == 'node':
                ...
                for i in range(len(way_nodes)-1):
                    ...
                    connections.append((way_nodes[i+1], way_nodes[i]))  # Conexión inversa
                elem.clear()
        ...
        return {
            ...
            'stats': {
                ...
                'total_edges': len(connections)//2  # Divide por 2 por las conexiones bidireccionales
            }
        }
...
def connect_components(graph):
    ...
    while len(components) > 1:
        ...
        if best_pair:
            ...
            print(f"Conectados {best_pair[0]} y {best_pair[1]} (distancia: {min_dist:.2f}m)")
        ...
        visualization_data = {
            'network': {
                ...
                'features': [{
                    ...
                    'geometry': {
                        'type': 'LineString',
                        'coordinates': [
                            [app.graph.nodes[n1]['pos'][1], app.graph.nodes[n1]['pos'][0]],
                            [app.graph.nodes[n2]['pos'][1], app.graph.nodes[n2]['pos'][0]]
                        ]
                    },
                    'properties': {
                        'color': '#cccccc',
            ...
            'points': {
                ...
                'features': [{
                    ...
                    'geometry': {
                        'type': 'Point',
                        'coordinates': [app.graph.nodes[node]['pos'][1], app.graph.nodes[node]['pos'][0]]
                    },
                    'properties': {
